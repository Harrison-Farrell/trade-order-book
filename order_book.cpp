...
int main(int argc, char* argv[]) {
  ...
  if (argc == 2) {
    ...
    std::cout << "Defaulting output stream to " << output_path << std::endl;
  } else if (argc == 4) {
    depth = std::stoi(argv[1]);
    input_path = argv[2];
    output_path = argv[3];
    std::cout << "Depth: " << depth << std::endl;
    std::cout << "Input stream: " << input_path << std::endl;
    std::cout << "Output stream: " << output_path << std::endl;
  } else {
    std::cout
      << "Incorrect number of arguments supplied.\n Defaulting to Depth of "
      << depth << std::endl;
    std::cout << "Defaulting input stream to " << input_path << std::endl;
    std::cout << "Defaulting output stream to " << output_path << std::endl;
  }
