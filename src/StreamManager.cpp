...
READER::StreamManager::StreamManager(const std::string& filePath, const std::string& outpath)
{
  this->reader_ = std::ifstream(filePath, std::ifstream::in | std::ifstream::binary);
  this->ouput_writer_ = std::ofstream(outpath, std::ofstream::out | std::ofstream::trunc);
  this->debug_writer_ = std::ofstream("Debug.log", std::ofstream::out | std::ofstream::trunc);

  if(this->reader_.fail())
...
size_t READER::StreamManager::GetLength()
{
  return this->input_stream_length_;
}
...
bool READER::StreamManager::IsOpen()
{
  return reader_.is_open();
}

size_t READER::StreamManager::Tell()
{
  return (size_t)this->reader_.tellg();
}
...
const MSG::Message READER::StreamManager::DecodeMessage()
{
  ...
  incoming_message.id.symbol[0] = (char)ReadUInt8();
  incoming_message.id.symbol[1] = (char)ReadUInt8();
  incoming_message.id.symbol[2] = (char)ReadUInt8();
  incoming_message.id.symbol[3] = '\0';
...
void READER::StreamManager::WriteLog(bool toTerminal, std::stringstream stream)
{
  if(toTerminal)
  {
